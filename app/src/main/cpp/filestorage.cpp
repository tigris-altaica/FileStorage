...
extern "C" {
    uint8_t key[16] = {0xe0, 0x4f, 0xd0, 0x20, 0xea, 0x3a, 0x69, 0x10, 0xa2, 0xd8, 0x08, 0x00, 0x2b, 0x30, 0x30, 0x9d};
    ...
    JNIEXPORT jbyteArray JNICALL Java_com_example_filestorage_MainActivity_encryptFile
            (JNIEnv *jenv, jobject self, jbyteArray jplaintext, jint jlen) {
        uint8_t* plaintext = (uint8_t*)jenv->GetByteArrayElements(jplaintext, NULL);
        int len = (int)jlen;

        int toPad = BLOCKSIZE - (len % BLOCKSIZE);
        plaintext[len] = (uint8_t)0x80;
        for (int i = 1; i < toPad; i++)
            plaintext[len + i] = (uint8_t)0x00;
        len += toPad;

        uint8_t* ciphertext = new uint8_t[len];

        aesEncrypt(key, 16, plaintext, ciphertext, len);

        jbyteArray ret = jenv->NewByteArray(len);
        jenv->SetByteArrayRegion(ret, 0, len, reinterpret_cast<const jbyte *>(ciphertext));
        return ret;
    }

    JNIEXPORT jbyteArray JNICALL Java_com_example_filestorage_MainActivity_decryptFile
            (JNIEnv *jenv, jobject self, jbyteArray jciphertext, jint jlen) {
        const uint8_t* ciphertext = (uint8_t*)jenv->GetByteArrayElements(jciphertext, NULL);
        int len = (int)jlen;

        uint8_t* plaintext = new uint8_t[len];

        aesDecrypt(key, 16, ciphertext, plaintext, len);
        ...
        jbyteArray ret = jenv->NewByteArray(len);
        jenv->SetByteArrayRegion(ret, 0, len, reinterpret_cast<const jbyte *>(plaintext));
        return ret;
    }
}